// Based on React Bits ProfileCard by Dominik Koch
...
const detectIOS = (): boolean => {
  ...
  // iPhone, iPad, iPod — and iPadOS 13+ reports as Mac with touch
  return /iPad|iPhone|iPod/.test(ua) ||
    (ua.includes('Mac') && typeof document !== 'undefined' && 'ontouchend' in document);
};

if (typeof document !== 'undefined') {
  // Inject keyframes + iOS override styles
  if (!document.getElementById('pc-keyframes')) {
    ...
    style.textContent = `
      ...
      html.is-ios .pc-name-text,
      html.is-ios .pc-title-text {
  ...
  // Set html.is-ios class before React renders (runs on module load, client-only)
  if (detectIOS()) {
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  if (!isMounted) return <div className={`${className} max-h-[400px] sm:max-h-[540px]`} style={{ aspectRatio: '0.718' }} />;
  ...
  return (
    ...
    >
      ...
      <div ref={shellRef} className="relative z-[1] group">
        <section
          className="grid relative overflow-hidden h-[65svh] sm:h-[80svh] max-h-[400px] sm:max-h-[540px]"
          style={{
            aspectRatio: '0.718',
            borderRadius: cardRadius,
          ...
          >
            ...
            <div className="overflow-visible" style={{ transform: 'translateZ(2px)', gridArea: '1 / -1', borderRadius: cardRadius, pointerEvents: 'none', backfaceVisibility: 'hidden' }}>
              ...
              <div className="absolute inset-0 z-[1]" style={{
                borderRadius: cardRadius,
                background: 'linear-gradient(to bottom, rgba(10,10,30,0.7) 0%, transparent 30%, transparent 60%, rgba(10,10,30,0.5) 100%)',
                pointerEvents: 'none',
              }} />
              ...
              <img
                className="w-[85%] max-h-[75%] sm:w-full sm:max-h-none object-contain absolute left-1/2 bottom-[-1px] will-change-transform transition-transform duration-[120ms] ease-out"
                src={avatarUrl}
                alt={`${name} avatar`}
                loading="lazy"
                style={{
              ...
              {showUserInfo && (
                ...
                >
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="rounded-full overflow-hidden border border-white/10 flex-shrink-0 w-[34px] h-[34px] sm:w-[48px] sm:h-[48px]">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                    ...
                    <div className="flex flex-col items-start gap-1 sm:gap-1.5">
                      ...
                      <div className="text-[11px] sm:text-sm text-white/70 leading-none">{status}</div>
                    </div>
                  </div>
                  <button
                    className="border border-white/10 rounded-lg px-3 py-2 sm:px-4 sm:py-3 text-[11px] sm:text-xs font-semibold text-white/90 cursor-pointer backdrop-blur-[10px] transition-all duration-200 ease-out hover:border-white/40 hover:-translate-y-px"
            ...
            <div className="pc-name-wrap max-h-full overflow-hidden text-center relative z-[5] mix-blend-luminosity"
              ...
              <div className="w-full absolute flex flex-col top-[1.2em] sm:top-[3em]">
                <h3 className="pc-name-text font-semibold m-0 whitespace-nowrap text-[clamp(18px,5.6vw,22px)] sm:text-[clamp(24px,4svh,32px)]"
